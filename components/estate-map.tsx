...
import { useTransitionRouter } from "@/components/view-transition";
 ...
 * Heimdall reading everything, the gateway as the only way out to a model,
 * Clarity on top. The five regions are the five Loweconex case studies;
 * hover one and it takes the accent while the rest recedes, and the caption
 * under the map becomes that study's claim and link. Click, and the title
 * travels into the page.
 *
 * Every label is a fact the case studies already state. Traffic moves along
 * the edges slowly; reduced motion stops it. Hidden under lg, where the
 * rows below carry the navigation.
 */
...
/** The first sentence of the outcome: one hook per region. */
const hook = (text: string) => text.split(/(?<=\.)\s+/)[0];
...
function Region({
  ...
  return (
    <a
      ...
      tabIndex={0}
      aria-label={`${p.title}: ${p.docCta ?? "case study"}`}
...
export function EstateMap() {
  ...
  return (
    <div className="hidden lg:block">
      <div className="estate max-w-[1100px]" data-hot={hot ?? undefined}>
        <svg
          viewBox="0 0 960 600"
          className="block h-auto w-full overflow-visible"
          role="group"
          aria-label="The platform built at Loweconex: the delivery pipeline, four Kubernetes environments, the observability stack, Heimdall, the AI gateway and Clarity. Each part links to its case study."
        >
          ...
          <g className="neutral">
            ...
            <text className="sub" x={196} y={470}>
              20 services × four environments · EKS, ArgoCD-reconciled
            </text>
          </g>

          <Region id="pipeline-platform" hot={hot} setHot={setHot}>
            ...
            <Flow d="M720 541 C 800 541, 882 500, 882 424" i={7} />
          ...
          <Region id="observability" hot={hot} setHot={setHot}>
            ...
            <text className="sub" x={700} y={72}>22 dashboards as code · 50+ alerts, a runbook each</text>
          </Region>

          <Region id="heimdall" hot={hot} setHot={setHot}>
          ...
          <Region id="ai-gateway" hot={hot} setHot={setHot}>
            <Flow d="M130 250 V222" i={15} />
            <path className="edge" d="M130 118 V92" />
            <rect className="node" x={40} y={222} width={200} height={46} />
            <text className="big" x={54} y={242}>AI Gateway</text>
            <text className="sub" x={54} y={259}>LiteLLM · allowlists per key</text>
            <rect className="node ext" x={72} y={62} width={116} height={30} />
            <text x={86} y={81}>Gemini · models</text>
            <text className="sub" x={40} y={52}>the only way out to a model</text>
          </Region>

          <Region id="clarity" hot={hot} setHot={setHot}>
            <Flow d="M130 176 V118" i={16} />
            <Flow d="M240 150 C 420 150, 640 220, 700 350" i={17} thin />
            <rect className="node" x={40} y={118} width={200} height={58} />
            <text className="big" x={54} y={140}>Clarity</text>
            <text className="sub" x={54} y={158}>English in, SQL that ran out</text>
            <text className="sub" x={54} y={172}>~30 tenants · no vector store</text>
          </Region>
        </svg>

        <p className="mt-3 flex min-h-5 items-baseline gap-3.5 font-mono text-xs text-muted-foreground/80" aria-live="polite">
          {active ? (
            <>
              <span className="rounded border border-primary/45 px-1.5 py-0.5 text-[10px] uppercase tracking-[0.16em] text-primary">
                {active.docType}
              </span>
              <span className="text-foreground">{hook(active.outcome ?? active.description)}</span>
              <span className="ml-auto whitespace-nowrap text-primary">{active.docCta ?? "case study"} →</span>
            </>
          ) : (
            <span>The platform I built and ran at Loweconex, 2023 to 2026. Hover any part of it.</span>
          )}
        </p>
      </div>
    </div>
